#ifndef _sz_huffman_hpp
#define _sz_huffman_hpp
...
void init(HuffmanTree *huffmanTree, const int *s, size_t length)
{
	size_t i, index;
	...
	{
		index = s[i];
		// printf("%ld/%ld : index/range = %ld/%d\n", i, length, index, huffmanTree->allNodes);
		// if(index >= huffmanTree->allNodes){
		// 	printf("%ld/%ld : index = %ld out of range %d\n", i, length, index, huffmanTree->allNodes);
		// }
		freq[index]++;
	}
...
void decode(const unsigned char *s, size_t targetLength, node t, int *out)
{
...
node reconstruct_HuffTree_from_bytes_anyStates(HuffmanTree *huffmanTree, const unsigned char* bytes, int nodeCount)
{
	if(nodeCount<=256)
	{
		unsigned char* L = (unsigned char*)malloc(nodeCount*sizeof(unsigned char));
...
void encode_withTree(HuffmanTree* huffmanTree, const int *s, size_t length, unsigned char **out, size_t *outSize)
{
	size_t i; 
	int nodeCount = 0;
